class GeniePopup extends HTMLElement {
  ...
  render() {
    ...
    template.innerHTML = `
      <style>
        ...
        /* During the genie animation the content is hidden and only the
           shrinking/expanding snapshot canvas paints the popup. Without
           this the underlying metaball field bleeds through the 0.78
           overlay as the snapshot shrinks, reading as a jarring light
           flash on close. Forcing the overlay opaque while animating
           keeps the backdrop solid throughout the genie. */
        ...
        /* Definition-list variant of the stack — used by the
           projects-addon renderer for a tighter two-column grid. */
        ::slotted(dl.infobox-stack) {
          margin: 0;
          padding: 0;
          display: grid;
  ...
  async snapshotWindow() {
    ...
    return await html2canvas(this.contentBox, {
      ...
      // 0.5× the source pixel density. The genie redraws the snapshot
      // per-row via drawImage at runtime scale, so halving the capture
      // resolution roughly quarters the html2canvas cost (which is the
      // dominant chunk of the perceived open-press delay) with no
      // visible quality hit against the soft metaball backdrop.
      scale: 0.5,
      useCORS: true,
      allowTaint: false,
